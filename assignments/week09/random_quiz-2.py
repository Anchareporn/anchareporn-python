...
def get_range_hint(number, current_min=1, current_max=100):
    ...
    print(f"HINT: The narrowed rage arount the number is {range(number-12,number+12)}")

def get_thefirst_digit_hint(number):
    # Retun the first digit of the number
    print(f"HINT: The first digit  arount the number is {result[0]}")

print("===Enhanced GUESSING GAME===")
print("Guess my number between 1 and 100!")
print("Yopu have unlimited attempts.")

random_number =int(input("Attempt .. - Enter your guess:"))


    if ramdom_number == guess_number:
        print(f"Congratulation! You won in {attempt}")
        break
